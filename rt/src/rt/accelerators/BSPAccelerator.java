...
import static rt.util.StaticVecmath.getDimension;
...
public class BSPAccelerator implements Intersectable {
	...
	private BSPNode buildTree(BSPNode node, List<Intersectable> iList, int depth) {
		...
		AxisAlignedBox minLeftBox = null, minRightBox = null;
		Axis minAxis = null;
		float minSplitDist = 0;
		AxisAlignedBox b = node.boundingBox;
		...
		for (Axis axis : StaticVecmath.Axis.values()) {
			...
			for (int step = -NR_SPLIT_TRIES; step <= NR_SPLIT_TRIES; step++) {
				...
				AxisAlignedBox leftBox = new AxisAlignedBox(new Point3f(b.min),
						leftBoxMax);
				AxisAlignedBox rightBox = new AxisAlignedBox(rightBoxMin, new Point3f(
						b.max));
