...
public class AxisAlignedBox implements Intersectable {

	final public static AxisAlignedBox INFINITE_BOUNDING_BOX = new AxisAlignedBox(new Point3f(Float.NEGATIVE_INFINITY,Float.NEGATIVE_INFINITY, Float.NEGATIVE_INFINITY), 
	...
	public AxisAlignedBox(Point3f bottomLeft, Point3f topRight) {
		this.min = bottomLeft;
		this.max = topRight;
	 ...
	 * @return
	 */
	public boolean isOverlapping(AxisAlignedBox other) {
	...
	public AxisAlignedBox transform(Matrix4f t) {
		Point3f instanceMin = new Point3f(this.min);
		Point3f instanceMax = new Point3f(this.max);
		t.transform(instanceMin);
		t.transform(instanceMax);
		return new AxisAlignedBox(instanceMin, instanceMax);
	}

	public AxisAlignedBox unionWith(AxisAlignedBox b) {
		Point3f unionMin = new Point3f(this.min);
		Point3f unionMax = new Point3f(this.max);
		StaticVecmath.elementwiseMax(unionMax, b.max);
		StaticVecmath.elementwiseMin(unionMin, b.min);
		return new AxisAlignedBox(unionMin, unionMax);
	}
